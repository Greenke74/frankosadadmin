...
const SetNewPasswordForm = () => {
  ...
  const {
    ...
    defaultValues: { password: "", passwordCopy: "" },
    mode: "onChange",
  });
      ...
      <FormControl variant="standard" fullWidth sx={{ marginBottom: 2 }}>
        ...
        {errors.password && (
          <ErrorMessage
            ...
            minLength={errors?.password?.type === "minLength" ? 6 : undefined}
          />
        )}
      </FormControl>
      <FormControl variant="standard" fullWidth>
        <StyledInputLabel shrink htmlFor="passwordCopyInput">
          Пароль ще раз
        </StyledInputLabel>
        <StyledInputBase
          placeholder="Ваш новий пароль"
          id="passwordCopyInput"
          type="password"
          {...register("passwordCopy", { minLength: 6, required: true })}
        />
        {errors.passwordCopy && (
          <ErrorMessage
            type={errors?.passwordCopy?.type}
            minLength={
              errors?.passwordCopy?.type === "minLength" ? 6 : undefined
            }
          />
        )}
      </FormControl>
